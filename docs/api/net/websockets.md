...
> ```ts
> static createWebsocket(uri: string, handler: string, connectHeaders?: { [name: string]: string }): WebsocketClient;
> ```
>
...
> | `connectHeaders` | `{ [name: string]: string }` | Optional headers of the STOMP CONNECT frame. A Dirigible `/stomp` endpoint needs `Authorization: Bearer <token>` there - an anonymous CONNECT is refused. |
>
> ::: info Returns
>

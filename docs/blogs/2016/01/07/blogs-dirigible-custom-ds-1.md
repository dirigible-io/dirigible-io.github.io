...
**On this stage**: We have setup a JNDI [javax.sql.DataSource](https://docs.oracle.com/javase/7/docs/api/javax/sql/DataSource.html) instance named **`jdbc/PostgreSQL`** that can be looked up and is fully capable of producing connections to a PostgreSQL database as defined by its parameters.
...
Locate Dirigible’s web.xml descriptor file and open it for edit. In web.xml, locate the section with the tag **`<servlet id="bridge">`** and scroll down to its set of init-param tags. Add the following block:
...
Make sure to use the prefix **`“jndiCustomDataSource-“`** in `param-name` exactly as is, case sensitive. The suffix **`postgre`** will be used as identity for this resource in the next step.

Notice, the construction of the string in the param-value. The pattern is to add the prefix **`java:comp/env`** to the name of the JNDI `javax.sql.DataSource` resource we defined in the previous step in `context.xml`. In our case this is the string **`jdbc/PostgreSQL`**.
...
In the dialog that pops up, locate ***Data Sources*** in the list on the left, and then click the button ***New…***

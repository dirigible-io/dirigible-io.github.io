...
Cross-origin access for frontends hosted outside the platform - a single-page application on static hosting, a mobile application, a desktop shell. Unset, the OAuth2 profiles answer no cross-origin request and the basic profile keeps allowing every origin without credentials. See [External frontends](/help/setup/authentication/external-frontends).

| Variable                           | Default                                              | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------------------------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DIRIGIBLE_CORS_ALLOWED_ORIGINS`   | -                                                    | Comma-separated origins or origin patterns that may call the platform cross-origin, e.g. `https://app.example.com,https://*.example.com,capacitor://localhost,tauri://localhost`. Setting it enables CORS on every profile. A port pattern is accepted (`:[8080]`, `:[*]`). A multi-port list `:[8080,8081]` cannot be written, since the value is split on commas - list each port as its own pattern. A pattern must name a host to reach the STOMP handshake - a pattern whose host is only wildcards (`*`, `https://*`, `https://**`) serves bearer clients over HTTP only. |
| `DIRIGIBLE_CORS_ALLOW_CREDENTIALS` | `false`                                              | Whether cross-origin requests may carry cookies. Refused together with a wildcard origin. Bearer-token clients do not need it. Does not govern the STOMP endpoint, which answers its own CORS with credentials - the origin list is what protects it.                                                                                                                                                                                                                                                                                                                           |
...
| `DIRIGIBLE_CORS_MAX_AGE`           | `3600`                                               | Seconds a browser may cache a preflight answer.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |

## Bearer tokens (cognito, keycloak)

How the OAuth2 login profiles treat a token presented in the `Authorization: Bearer` header. ID tokens identify a user and are verified like a login. Access tokens keep the rules machine clients relied on before. See [External frontends](/help/setup/authentication/external-frontends).

| Variable                                      | Default                                                                              | Purpose                                                                                                             |
| --------------------------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------- |
| `DIRIGIBLE_OAUTH2_JWT_TOKEN_KINDS`            | `id,access`                                                                          | Kinds of tokens accepted: `id` (user identity, group roles) and `access` (client identity from `sub`, scope roles). |
| `DIRIGIBLE_OAUTH2_JWT_PRINCIPAL_CLAIM`        | Profile's user-name attribute (`email` on Cognito, `preferred_username` on Keycloak) | Claim of an ID token the user name is read from. A token without it is refused.                                     |
| `DIRIGIBLE_OAUTH2_JWT_AUDIENCES`              | The profile's client id                                                              | Audiences a token must be issued for. ID tokens are always checked. Access tokens only once this is set.            |
| `DIRIGIBLE_OAUTH2_JWT_ISSUER_URI`             | The profile's issuer                                                                 | Issuer every token must carry.                                                                                      |
| `DIRIGIBLE_OAUTH2_JWT_JWK_SET_URI`            | The profile's JWKS endpoint                                                          | Where the signing keys are fetched from.                                                                            |
| `DIRIGIBLE_OAUTH2_JWT_REQUIRE_VERIFIED_EMAIL` | `true`                                                                               | An ID token identified by `email` must also carry `email_verified=true`.                                            |

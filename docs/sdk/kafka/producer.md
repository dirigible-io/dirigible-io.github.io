...
### Key Features:
- **Single-record Publish**: One method, one record &mdash; key and value are passed as `String` and serialized by the configured Kafka serializers.
...
Publishes a single record to the named topic. Two overloads are provided &mdash; one taking an explicit Kafka producer configuration JSON, one falling back to the platform default.
...
> | `key` | `String` | Record key &mdash; used for partitioning. |
> | `value` | `String` | Record value &mdash; the message payload. |

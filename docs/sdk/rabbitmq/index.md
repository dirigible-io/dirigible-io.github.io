...
- [`Producer`](./producer.md) &mdash; publishes a single message to a RabbitMQ queue using the platform's default channel configuration.
- [`Consumer`](./consumer.md) &mdash; subscribes a registry handler script to a RabbitMQ queue, and stops the subscription when done.

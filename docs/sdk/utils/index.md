...
- **Utf8**: UTF-8 `byte[]` <-> `String` conversion helpers.
- **Uuid**: Version 4 UUID generation and string-format validation.
- **Xml**: XML <-> JSON conversion via the platform's Jackson XML mapper.
...
- [`Utf8`](./utf8.md) - UTF-8 `byte[]` <-> `String` helpers.
- [`Uuid`](./uuid.md) - UUID generation and validation.
- [`Xml`](./xml.md) - XML <-> JSON conversion helpers.
